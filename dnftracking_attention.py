...
timescale = 1
start_time = 223015487 # events[0][0]
dsec = 0
...
def update_stimulus():
    global current_index, dnf, start_time, dsec, display, stimulus, saliency, next_frame_tmstmp, current_frame
...
def updatefig(*args):
    ...
    res = np.zeros((attention_input.shape[0]//20, attention_input.shape[1]//20))
    px = np.vsplit(attention_input, attention_input.shape[0]//20)
    for i in range(len(px)):
        px2 = np.hsplit(px[i], attention_input.shape[1]//20)
    ...
    att_pot.set_data(dnf_att.memory)
...
if __name__ == '__main__':
    ...
    att_pot = plt.imshow(dnf_att.memory, vmin=0, vmax=1) #, cmap='hot', interpolation='nearest', animated=True)
